...
def ask_for_money(request, host_id=2, favortype=None, invoice=None, ): #pass teh favor type and invoice in the URL pattern (www.blocbox.co/transactions/payment/
    ...
    if enduser.account_balance:
        if enduser.account_balance >= trans_created.price:
            ...
            payment_not_needed = False
        else:
            remaining_balance = 0
            payment_not_needed = True
    else:
    ...
    return render(request, 'blocbox/payment_enter_paypal.html', {
		    'enduser':enduser, 'host':host, 'invoice': invoice, 'remaining_balance': remaining_balance, 'payment_not_needed': payment_not_needed,
