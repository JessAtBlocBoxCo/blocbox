...
from transactions.forms import TrackingForm, ModifyTransaction, PackageReceived, EndUserIssue, MessageHost, HostReceived, HostHandoff, HostIssue, MessageUser
...
def dashboard_host_nothing(request, confirm_id=None):
    ...
    def dashboard_host(request, trans=None, track_id=None, confirm_id=None, handoff_id=None, issue_id=None, message_trans_id=None, archive_id=None, days_until_next_package=None):
    thepersonviewingthepage = request.user
    ...
    if thepersonviewingthepage.host == True:
        ...
        shipments_waiting = sorted(chain(shipments_waiting_pickup_delivered, shipments_waiting_pickup_received), key=attrgetter('last_tracking_datetime'), reverse=True)
        shipments_waiting_count = shipments_waiting_pickup_delivered.count()
        #Host connections
...
def host_received_modal(request, confirm_id):
    ...
    if request.method == 'POST':
        ...
        if host_received_form.is_valid():
            trans.host_received_comments = host_received_form.cleaned_data['host_received_comments']
